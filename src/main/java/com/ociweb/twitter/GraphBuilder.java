...
public class GraphBuilder  {
	...
	public void buildGraph(GraphManager gm, String[] queryText, int[] queryRoutes, String consumerKey, String consumerSecret) {
		...
		for(User a: users) {
			...
			//these uniques must be sent to the right rest module and added to table so they can be looked up.
			LongHashTable.setItem(table, a.id, c);
			subscriptions[c++] = tweets[0];

		}

		ServerPipesConfig serverConfig = new ServerPipesConfig(false, isTLS, 1);
		ServerCoordinator serverCoord = new ServerCoordinator(isTLS, bindHost, bindPort,
				serverConfig,"/follow?user=1234");


		NetGraphBuilder.buildHTTPServerGraph(gm, new RestModules(this, subscriptions, table, staticFilesPathRootIndex), serverCoord, serverConfig);
